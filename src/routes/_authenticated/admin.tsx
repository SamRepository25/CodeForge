...
import { getAdminAuditEntries, recordAdminAudit } from "@/lib/admin-audit";
...
type ContactAttachment = { id: string; file_name: string; mime_type: string; size_bytes: number; storage_path: string };
type ContactMessage = { id: string; name: string; email: string; subject: string | null; message: string; status: string; created_at: string; contact_attachments: ContactAttachment[] };
type Project = { id: string; title: string; slug: string; description: string; long_description: string | null; image_url: string | null; tags: string[]; category: string | null; github_url: string | null; live_url: string | null; featured: boolean; order_index: number; created_at: string };
type TimelineItem = { id: string; year: string; title: string; org: string; description: string; order_index: number | null };
...
function Admin() {
  ...
  const messages = useQuery({ queryKey: ["admin-messages"], queryFn: async () => { const { data, error } = await supabase.from("contact_messages").select("id,name,email,subject,message,status,created_at,contact_attachments(id,file_name,mime_type,size_bytes,storage_path)").order("created_at", { ascending: false }); if (error) throw error; return (data ?? []) as ContactMessage[]; } });
  const posts = useQuery({ queryKey: ["admin-posts"], queryFn: async () => { const { data, error } = await supabase.from("posts").select("*").order("created_at", { ascending: false }); if (error) throw error; return data ?? []; } });
  const projects = useQuery({ queryKey: ["admin-projects"], queryFn: async () => { const { data, error } = await supabase.from("projects").select("*").order("order_index", { ascending: true }); if (error) throw error; return (data ?? []) as Project[]; } });
  const education = useQuery({ queryKey: ["admin-education"], queryFn: async () => { const { data, error } = await supabase.from("education").select("*").order("order_index", { ascending: true }); if (error) throw error; return (data ?? []) as TimelineItem[]; } });
  const experience = useQuery({ queryKey: ["admin-experience"], queryFn: async () => { const { data, error } = await supabase.from("experience").select("*").order("order_index", { ascending: true }); if (error) throw error; return (data ?? []) as TimelineItem[]; } });
  const comments = useQuery({ queryKey: ["admin-comments"], queryFn: async () => { const { data, error } = await supabase.from("guest_comments").select("*").order("created_at", { ascending: false }); if (error) throw error; return data ?? []; } });
  const settings = useQuery({ queryKey: ["admin-site-settings"], queryFn: async () => { const { data, error } = await supabase.from("site_settings").select("*").order("key", { ascending: true }); if (error) throw error; return data ?? []; } });
  const media = useQuery({ queryKey: ["admin-media"], queryFn: async () => { const { data, error } = await supabase.storage.from("site-media").list("", { limit: 100, sortBy: { column: "updated_at", order: "desc" } }); if (error) throw error; return (data ?? []) as MediaFile[]; } });
  const refreshAll = () => { void Promise.all([messages.refetch(), posts.refetch(), projects.refetch(), education.refetch(), experience.refetch(), comments.refetch(), settings.refetch(), media.refetch()]); };
  const stats: AdminStats = useMemo(() => ({ posts: posts.data?.length ?? 0, published: posts.data?.filter((p) => p.published).length ?? 0, projects: projects.data?.length ?? 0, messages: messages.data?.length ?? 0, unread: messages.data?.filter((m) => m.status === "unread").length ?? 0, comments: comments.data?.length ?? 0, pendingComments: comments.data?.filter((c) => !c.approved).length ?? 0, views: posts.data?.reduce((sum, p) => sum + (p.views ?? 0), 0) ?? 0 }), [posts.data, projects.data, messages.data, comments.data]);
  const recordActivity = async (action: string, resource: string, resourceId?: string) => { if (!user) return; await recordAdminAudit(action, resource, resourceId); };
  const selectTab = (value: string) => setTab(value as AdminTab);

  return <SiteLayout><section className="mx-auto max-w-[1500px] px-4 pt-8 pb-20">
    <div className="flex flex-wrap items-center justify-between gap-4"><Link to="/dashboard" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Dashboard</Link><Button size="sm" variant="outline" className="rounded-xl" onClick={refreshAll}><RefreshCw className="mr-1.5 h-4 w-4" /> Refresh</Button></div>
    <div className="mt-5"><div className="text-xs uppercase tracking-[0.18em] text-electric">Private administration</div><h1 className="mt-2 font-display text-4xl font-bold">CodeForge Control Center</h1><p className="mt-2 max-w-3xl text-sm text-muted-foreground">Manage content, messages, portfolio data, security, media and operational checks from one admin surface.</p></div>
    <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">{[["Posts",stats.posts,"posts"],["Published",stats.published,"posts"],["Projects",stats.projects,"projects"],["Messages",stats.messages,"messages"],["Unread",stats.unread,"messages"],["Comments",stats.comments,"comments"],["Pending",stats.pendingComments,"comments"],["Views",stats.views,"overview"]].map(([label,value,target]) => <button key={String(label)} type="button" onClick={() => setTab(target as AdminTab)} className="glass rounded-2xl p-4 text-left transition hover:-translate-y-0.5"><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 font-display text-2xl font-bold gradient-text">{value}</div></button>)}</div>
    <div className="mt-8 overflow-x-auto pb-2"><div className="flex min-w-max gap-1 rounded-2xl border border-border/40 bg-background/40 p-1">{navItems.map(([value,label,Icon]) => <button key={value} type="button" onClick={() => setTab(value)} className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium transition ${tab === value ? "bg-electric text-background" : "text-muted-foreground hover:bg-muted/60 hover:text-foreground"}`}><Icon className="h-3.5 w-3.5" /> {label}</button>)}</div></div>
    <div className="mt-6">
      ...
      {tab === "comments" && <CommentsPanel comments={comments.data ?? []} refetch={comments.refetch} onActivity={recordActivity} />}
      ...
      {tab === "trash" && <TrashPanel messages={messages.data ?? []} refetch={messages.refetch} onActivity={recordActivity} />}
...
function Messages({ messages, refetch, onActivity }: { messages: ContactMessage[]; refetch: () => Promise<unknown>; onActivity: (a: string, r: string, id?: string) => Promise<void>; }) {
  ...
  const markRead = async (m: ContactMessage) => { if (m.status !== "unread") return; const { error } = await supabase.from("contact_messages").update({ status: "read" }).eq("id", m.id); if (error) return toast.error(error.message); await onActivity("read", "contact_message", m.id); await refetch(); };
...
function Posts({ posts, refetch, onActivity }: { posts: Array<Record<string, unknown>>; refetch: () => Promise<unknown>; onActivity: (a: string, r: string, id?: string) => Promise<void>; }) {
  const [editing, setEditing] = useState<PostDraft | undefined>(); const [creating, setCreating] = useState(false); const typedPosts = posts as Array<PostDraft & { id: string; created_at: string; views: number }>;
...
function Projects({ projects, refetch, onActivity }: { projects: Project[]; refetch: () => Promise<unknown>; onActivity: (a: string, r: string, id?: string) => Promise<void>; }) {
  const empty: Project = { id: "", title: "", slug: "", description: "", long_description: "", image_url: "", tags: [], category: "", github_url: "", live_url: "", featured: false, order_index: 0, created_at: "" }; const [draft, setDraft] = useState<Project | null>(null); const [tagText, setTagText] = useState("");
...
function TimelineManager({ title, table, items, refetch, onActivity }: { title: string; table: "education" | "experience"; items: TimelineItem[]; refetch: () => Promise<unknown>; onActivity: (a: string, r: string, id?: string) => Promise<void>; }) {
  const empty: TimelineItem = { id: "", year: "", title: "", org: "", description: "", order_index: 0 }; const [draft, setDraft] = useState<TimelineItem | null>(null);
...
function CommentsPanel({ comments, refetch, onActivity }: { comments: Array<{ id: string; name: string; email: string; content: string; approved: boolean; created_at: string }>; refetch: () => Promise<unknown>; onActivity: (a: string, r: string, id?: string) => Promise<void> }) {
  const update = async (id: string, approved: boolean) => {
    const { error } = await supabase.from("guest_comments").update({ approved }).eq("id", id);
    if (error) return toast.error(error.message);
    await onActivity(approved ? "approve" : "hide", "guest_comment", id);
  ...
  const remove = async (id: string) => {
    ...
    if (error) return toast.error(error.message);
    await onActivity("delete", "guest_comment", id);
...
function EngagementPanel({ posts }: { posts: Array<{ id: string; title: string; views: number }> }) { const [counts,setCounts]=useState<Record<string,number>>({}); useEffect(() => { let alive=true; void Promise.all(posts.map(async(p)=>{const {data}=await supabase.rpc("get_post_like_count",{_post_id:p.id}); return [p.id,data ?? 0] as const;})).then((pairs)=>{if(alive)setCounts(Object.fromEntries(pairs));}); return()=>{alive=false;}; },[posts]); return <Panel title="Engagement" description="Post-level views and like totals, using the existing Supabase aggregate function for likes."><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b border-border/40 text-left text-xs uppercase tracking-wide text-muted-foreground"><th className="px-3 py-3">Post</th><th className="px-3 py-3">Views</th><th className="px-3 py-3">Likes</th></tr></thead><tbody>{posts.map((p)=><tr key={p.id} className="border-b border-border/20"><td className="px-3 py-3 font-medium">{p.title}</td><td className="px-3 py-3">{p.views}</td><td className="px-3 py-3">{counts[p.id] ?? "…"}</td></tr>)}</tbody></table>{posts.length===0&&<Empty label="No posts."/>}</div></Panel>; }
function BookmarksPanel({ posts }: { posts: Array<{ id: string; title: string; slug: string }> }) { return <Panel title="Bookmarks" description="Bookmark records are intentionally user-private under RLS. This view preserves that privacy instead of exposing other users' private bookmarks."><p className="text-sm text-muted-foreground">Use the normal Dashboard → Bookmarks view to manage the signed-in account's bookmarks.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{posts.slice(0,6).map((p)=><Link key={p.id} to="/blogs/$slug" params={{slug:p.slug}} className="glass rounded-xl p-4"><div className="font-medium">{p.title}</div><div className="mt-1 text-xs text-electric">Open article →</div></Link>)}</div></Panel>; }
function GlobalSearch({ posts, projects, messages }: { posts: Array<{ id: string; title: string; slug: string; content: string }>; projects: Project[]; messages: ContactMessage[] }) { const [query,setQuery]=useState(""); const q=query.trim().toLowerCase(); const results=q?[...posts.filter((p)=>`${p.title} ${p.content}`.toLowerCase().includes(q)).map((p)=>({type:"Post",title:p.title,slug:p.slug})),...projects.filter((p)=>`${p.title} ${p.description} ${p.tags.join(" ")}`.toLowerCase().includes(q)).map((p)=>({type:"Project",title:p.title})),...messages.filter((m)=>`${m.name} ${m.email} ${m.subject??""} ${m.message}`.toLowerCase().includes(q)).map((m)=>({type:"Message",title:`${m.subject||"No subject"} — ${m.name}`}))]:[]; return <Panel title="Global search" description="Search posts, projects and contact messages visible to the administrator."><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><Input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search CodeForge…" className="rounded-xl pl-9"/></div><div className="mt-5 divide-y divide-border/30">{results.map((r,i)=><div key={`${r.type}-${i}`} className="py-3"><div className="text-xs text-electric">{r.type}</div>{r.type==="Post"?<Link to="/blogs/$slug" params={{slug:r.slug}} className="font-medium hover:text-electric">{r.title}</Link>:<div className="font-medium">{r.title}</div>}</div>)}{q&&results.length===0&&<Empty label="No results."/>}{!q&&<div className="py-8 text-center text-sm text-muted-foreground">Start typing to search.</div>}</div></Panel>; }
function HealthPanel() { const [status,setStatus]=useState<Record<string,string>>({}); const check=async()=>{setStatus({}); const started=Date.now(); const checks=await Promise.all([supabase.from("profiles").select("id",{count:"exact",head:true}),supabase.from("posts").select("id",{count:"exact",head:true}),supabase.storage.from("site-media").list("",{limit:1})]); setStatus({Database:checks[0].error?"Error":"Healthy",Posts:checks[1].error?"Error":"Healthy",Storage:checks[2].error?"Check migration":"Healthy",Latency:`${Date.now()-started} ms`});}; useEffect(()=>{void check();},[]); return <Panel title="System health" description="Read-only production checks. No secrets or service-role credentials are exposed."><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Object.entries(status).map(([k,v])=><div key={k} className="glass rounded-2xl p-5"><div className="text-xs text-muted-foreground">{k}</div><div className={`mt-1 font-semibold ${v==="Healthy"?"text-emerald-400":"text-amber-400"}`}>{v}</div></div>)}</div><Button variant="outline" className="mt-5 rounded-xl" onClick={()=>void check()}><RefreshCw className="mr-1.5 h-4 w-4"/>Run checks</Button></Panel>; }
function ExportPanel({ posts, projects, messages }: { posts: Array<Record<string,unknown>>; projects: Project[]; messages: ContactMessage[] }) { const downloadCsv=(name:string,rows:Array<Record<string,unknown>>)=>{if(!rows.length)return toast.error("Nothing to export");const keys=Array.from(new Set(rows.flatMap((r)=>Object.keys(r))));const esc=(value:unknown)=>`"${String(value??"").replaceAll('"','""')}"`;const csv=[keys.join(","),...rows.map((r)=>keys.map((k)=>esc(r[k])).join(","))].join("\n");const blob=new Blob([csv],{type:"text/csv;charset=utf-8"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=name;a.click();URL.revokeObjectURL(url);}; return <Panel title="Export" description="Download administrator-visible content as CSV files. Exports are generated locally in the browser."><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Button variant="outline" className="h-auto justify-start rounded-xl p-4" onClick={()=>downloadCsv("codeforge-posts.csv",posts)}><FileText className="mr-3 h-5 w-5 text-electric"/><span><strong className="block">Posts</strong><span className="text-xs text-muted-foreground">{posts.length} records</span></span></Button><Button variant="outline" className="h-auto justify-start rounded-xl p-4" onClick={()=>downloadCsv("codeforge-projects.csv",projects)}><Briefcase className="mr-3 h-5 w-5 text-electric"/><span><strong className="block">Projects</strong><span className="text-xs text-muted-foreground">{projects.length} records</span></span></Button><Button variant="outline" className="h-auto justify-start rounded-xl p-4" onClick={()=>downloadCsv("codeforge-messages.csv",messages.map((m)=>({id:m.id,name:m.name,email:m.email,subject:m.subject,message:m.message,status:m.status,created_at:m.created_at})))}><Mail className="mr-3 h-5 w-5 text-electric"/><span><strong className="block">Messages</strong><span className="text-xs text-muted-foreground">{messages.length} records</span></span></Button></div></Panel>; }
function TrashPanel({ messages, refetch, onActivity }: { messages: ContactMessage[]; refetch: () => Promise<unknown>; onActivity: (a: string, r: string, id?: string) => Promise<void> }) {
  const archived=messages.filter((m)=>m.status==="archived");
  const restore=async(id:string)=>{
    const{error}=await supabase.from("contact_messages").update({status:"read"}).eq("id",id);
    if(error)return toast.error(error.message);
    await onActivity("restore","contact_message",id);
