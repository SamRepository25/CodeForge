...
export const loginWithProtection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // This remains an admin-only login endpoint. Non-admin accounts are not authenticated
    // or granted a session; their attempts are recorded by the existing login lockout.
    if (!isAdminEmail) {
      const { data: failureRows, error: failureError } = await supabaseAdmin.rpc(
        "login_lockout_record_failure",
      );
      if (failureError) {
        console.error("[login] non-admin attempt recording failed", failureError);
      ...
      return {
        success: false,
        reason: locked ? ("locked" as const) : ("not_admin" as const),
        locked,
        lockedUntil: locked ? lockedUntil : null,
