...
const EMPTY_ENTRY: Partial<Entry> = {
  year: "",
  title: "",
  org: "",
  description: "",
  ...
  return (
    <div className="glass gradient-border mt-4 rounded-2xl p-5">
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          onClick={onDone}
          className="flex items-center gap-1.5 rounded-xl border border-border/60 px-4 py-2 text-sm hover:bg-white/5"
        >
...
function EntrySection({
  ...
  const [form, setForm] = useState<{ open: boolean; entry: Partial<Entry> | null }>({
    open: false,
    entry: null,
  });
  const qc = useQueryClient();
    ...
    <section className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="mt-8 space-y-4">
        {entries.map((e) => (
          ...
          >
            <div
              className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br ${gradient}`}
            >
            ...
            {editMode && (
              <div className="flex shrink-0 flex-col gap-1.5 opacity-0 transition group-hover:opacity-100">
                <button
                  onClick={() => setForm({ open: true, entry: e })}
                  className="rounded-lg bg-violet/20 p-1.5 text-violet hover:bg-violet/30"
                >
        ...
        <div className="grid items-center gap-10 md:grid-cols-[1fr_auto]">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-2">
              ...
              <Button asChild variant="outline" className="rounded-xl">
                <a
                  href={s.github_url || "https://github.com/SamRepository25/"}
                  target="_blank"
                  rel="noreferrer"
                >
          ...
          <div className="float-slow relative h-56 w-56 overflow-hidden rounded-[2.5rem]">
            <picture>
              <source srcSet="/images/about-illustration.webp" type="image/webp" />
              <img
      ...
      <section className="mx-auto max-w-5xl px-4 py-12">
        <div className="glass gradient-border rounded-3xl p-10 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Open to collaborations, freelance work and conversations.
          </p>
